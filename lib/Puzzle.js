...
class Puzzle {
  ...
  generatePuzzles() {
    ...
    for (var i = 0; i < puzzleArray.length; i++) {
      var puzzleName = puzzleArray[i];
      for (var j = 0; j < data.puzzles[puzzleName].puzzle_bank.length; j++) {
        puzzles.push(data.puzzles[puzzleName].puzzle_bank[j]);
      }
    }
    return puzzles;
  }
  ...
  solvePuzzle() {
    ...
    if (userAnswer === correctAnswer) {
      ...
      var points = 20 * correctAnswer.length;
